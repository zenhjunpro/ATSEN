...
def validation(args, model, tokenizer, labels, pad_token_label_id, best_dev, best_test, 
    ...
    results, _, best_test, is_updated2 = evaluate(args, model, tokenizer, labels, pad_token_label_id, best_test, mode="train_xr", \
...
def random_sampler(args, label_, prob=None):#将一些lable为0的标记随机mask
    # label: batch, seq_len
...
def train(args, train_dataset, tokenizer, labels, pad_token_label_id):
    ...
    # optimizer_s1 = optim.SGD(model_s1.parameters(),
    #                       lr=0.05,
    #                       momentum=0.9,
    #                       weight_decay=5e-4,
    #                       nesterov=False)

    # optimizer_s2 = optim.SGD(model_s2.parameters(),
    #                       lr=0.05,
    #                       momentum=0.9,
    #                       weight_decay=5e-4,
    #                       nesterov=False)

    save_path = "/home/zengjun/SCDL/ptms/conll03/student1"
    model_path = os.path.join(save_path,"checkpoint-best-2")
    model_test = RobertaForTokenClassification_Modified.from_pretrained(model_path)
    tokenization_test = RobertaTokenizer.from_pretrained(model_path)
    model_test.to(args.device) 
    s1_best_dev, s1_best_test, _ = validation(args, model_test, tokenization_test, labels, pad_token_label_id, \
    s1_best_dev, s1_best_test, global_step, t_total, 0, "student1")
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if epoch >= args.begin_epoch:
                ...
                if delta//args.self_learning_period > 0:
                    if delta%args.self_learning_period == 0:
                        ...
                        # ###
                        # module_list.append(self_learning_teacher_model1)
                        # ###
                        self_learning_teacher_model1.eval()
                        self_learning_teacher_model2 = copy.deepcopy(t_model2)
                        # ###
                        # module_list.append(self_learning_teacher_model2)
                        # ###
                    ...
                    # Try some new Model
                    # sts_model1 = M2Teacher(t_model1,self_learning_teacher_model1,t_total,args,current_train_iter)
                    # sts_model2 = M2Teacher(t_model2,self_learning_teacher_model2,t_total,args,current_train_iter)
                    TEST = True
                    with torch.no_grad():
                        outputs1 = self_learning_teacher_model1(**inputs)
                        outputs2 = self_learning_teacher_model2(**inputs)
                        # outputs1 = sts_model1(inputs,update_param=True)
                        # outputs2 = sts_model2(inputs,update_param=True)
                ...
                with torch.no_grad():
                    ...
                    # label_mask1 = (torch.ones(pred_labels1.size(0),pred_labels1.size(1))) == 1
                    # label_mask2 = (torch.ones(pred_labels2.size(0),pred_labels2.size(1))) == 1
                    # print(label_mask1==label_mask2)
            ...
            if (step+1)%args.gradient_accumulation_steps == 0:
                ...
                # else:
                _update_mean_model_variables_STS(model_s1, model_t1, args.mean_alpha, global_step,t_total,args.al)
                _update_mean_model_variables_STS(model_s2, model_t2, args.mean_alpha, global_step,t_total,args.al)
